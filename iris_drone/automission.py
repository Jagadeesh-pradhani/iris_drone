...
input: GPS coordinat of target location and altitude
output: Drone will move to the target location with error of 2m
get location function,
gives the GPS location of target by taking input as Distance to the target, and current locatrion
"""
...
"""
Velocity functions,
send_ned_velocity(): Function to go in certain direction with required velocity in m/s
send_ned_velocity1(): Function to go to certain altitude with required velocity.
condition_yaw:- to yaw at required angle, heading->destination angle
"""
...
"""
vel:-velocity approach
target function: Goes towrds the target based on the calculated distance.
convert_frame function: converts the default direction frame to drone's direction frame
decend function: decends the altitude to drop payload
drop_payload function: drops the payload by triggering a servo using PWM signal
"""


def drop_payload(num, PWM):
    msg = vehicle.message_factory.command_long_encode(
...
def mission():
    ...
    #goto_location_norm(15.3677,75.1252397,alt)
    time.sleep(10)
    #vel()
    #decend(1)
